    ...
    for a in addrs:
        ...
        try:
            ...
            for o in jsonobj["unspent_outputs"]:
                tx_hash = o['tx_hash']
                h = "".join([tx_hash[i]+tx_hash[i+1] for i in range(0,len(tx_hash), 2)][::-1])
                u.append({
...
def bci_pushtx(tx):
    if not re.match('^[0-9a-fA-F]*$', tx):
        tx = tx.encode('hex')
    return make_request('https://blockchain.info/pushtx', ('tx='+tx).encode("utf-8"))
...
def blockr_pushtx(tx, network='btc'):
    ...
    return make_request(blockr_url, ('{"hex":"%s"}' % tx).encode("utf-8"))
